...
export function GigListingsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['gig', 'gig_listings', page, search],
    queryFn: () => apiFetch<any>(`/api/gig/gig_listings?page=${page}&limit=20${search ? `&q=${search}` : ''}`),
  });

  const createMutation = useMutation({
    mutationFn: (body: typeof form) => apiFetch<any>('/api/gig/gig_listings', { method: 'POST', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'gig_listings'] });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: number; body: typeof form }) =>
      apiFetch<any>(`/api/gig/gig_listings/${id}`, { method: 'PUT', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'gig_listings'] });
      setEditId(null);
      setForm({ name: '', description: '', status: 'active' });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/api/gig/gig_listings/${id}`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['gig', 'gig_listings'] }),
  });
  ...
  return (
    <div>
      <div className="page-header flex items-center justify-between">
        <div>
          <h1 className="page-title">GigListings</h1>
          <p className="page-subtitle">Gig Listings</p>
        </div>
      ...
      {/* Search */}
      <div className="mb-4">
        <input className="input max-w-sm" placeholder="Search gig listings..."
      ...
      {isLoading ? (
        ...
        <div className="card empty-state">
          ...
          <p className="empty-state-text">No gig listings found</p>
