...
export function ReviewsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['gig', 'contracts', page, search],
    queryFn: () => apiFetch<any>(`/api/gig/contracts?page=${page}&limit=20${search ? `&q=${search}` : ''}`),
  });

  const createMutation = useMutation({
    mutationFn: (body: typeof form) => apiFetch<any>('/api/gig/contracts', { method: 'POST', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'contracts'] });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: number; body: typeof form }) =>
      apiFetch<any>(`/api/gig/contracts/${id}`, { method: 'PUT', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'contracts'] });
      setEditId(null);
      setForm({ name: '', description: '', status: 'active' });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/api/gig/contracts/${id}`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['gig', 'contracts'] }),
  });
    ...
    <div>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Payment Processing</p>
        </div>
      ...
      {/* Search */}
      <div className="mb-4">
        <input className="input max-w-sm" placeholder="Search contracts..."
      ...
      {isLoading ? (
        ...
        <div className="card empty-state">
          ...
          <p className="empty-state-text">No contracts found</p>
