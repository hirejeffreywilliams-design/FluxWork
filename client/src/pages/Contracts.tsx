...
export function ContractsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['gig', 'skills', page, search],
    queryFn: () => apiFetch<any>(`/api/gig/skills?page=${page}&limit=20${search ? `&q=${search}` : ''}`),
  });

  const createMutation = useMutation({
    mutationFn: (body: typeof form) => apiFetch<any>('/api/gig/skills', { method: 'POST', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'skills'] });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: number; body: typeof form }) =>
      apiFetch<any>(`/api/gig/skills/${id}`, { method: 'PUT', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'skills'] });
      setEditId(null);
      setForm({ name: '', description: '', status: 'active' });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/api/gig/skills/${id}`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['gig', 'skills'] }),
  });
    ...
    <div>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Skill Matching</p>
        </div>
      ...
      {/* Search */}
      <div className="mb-4">
        <input className="input max-w-sm" placeholder="Search skills..."
      ...
      {isLoading ? (
        ...
        <div className="card empty-state">
          ...
          <p className="empty-state-text">No skills found</p>
