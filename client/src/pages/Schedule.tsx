...
export function SchedulePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['gig', 'reviews', page, search],
    queryFn: () => apiFetch<any>(`/api/gig/reviews?page=${page}&limit=20${search ? `&q=${search}` : ''}`),
  });

  const createMutation = useMutation({
    mutationFn: (body: typeof form) => apiFetch<any>('/api/gig/reviews', { method: 'POST', body }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: number; body: typeof form }) =>
      apiFetch<any>(`/api/gig/reviews/${id}`, { method: 'PUT', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'reviews'] });
      setEditId(null);
      setForm({ name: '', description: '', status: 'active' });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/api/gig/reviews/${id}`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['gig', 'reviews'] }),
  });
    ...
    <div>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Portfolio Display</p>
        </div>
      ...
      {/* Search */}
      <div className="mb-4">
        <input className="input max-w-sm" placeholder="Search reviews..."
      ...
      {isLoading ? (
        ...
        <div className="card empty-state">
          ...
          <p className="empty-state-text">No reviews found</p>
