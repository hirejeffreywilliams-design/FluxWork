...
export function PortfolioPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['gig', 'payments', page, search],
    queryFn: () => apiFetch<any>(`/api/gig/payments?page=${page}&limit=20${search ? `&q=${search}` : ''}`),
  });

  const createMutation = useMutation({
    mutationFn: (body: typeof form) => apiFetch<any>('/api/gig/payments', { method: 'POST', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'payments'] });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: number; body: typeof form }) =>
      apiFetch<any>(`/api/gig/payments/${id}`, { method: 'PUT', body }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['gig', 'payments'] });
      setEditId(null);
      setForm({ name: '', description: '', status: 'active' });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetch(`/api/gig/payments/${id}`, { method: 'DELETE' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['gig', 'payments'] }),
  });
    ...
    <div>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Review System</p>
        </div>
      ...
      {/* Search */}
      <div className="mb-4">
        <input className="input max-w-sm" placeholder="Search payments..."
